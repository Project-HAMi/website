  ...
  return (
    <ul className="support-wrapper casestudies-wrapper">
      {caseStudiesData.map(({ logo, logoZh, name, metric, url }, index) => {
        ...
        const logoPath =
          hasLogo && rawLogo.startsWith("/") ? rawLogo : `/img/casestudies/${rawLogo}`;
        const displayMetric = typeof metric === "object" ? (isZh ? metric.zh : metric.en) : metric;

        return (
          <li key={index}>
            <a href={url} target="_blank" rel="noopener noreferrer" className="casestudy-card-link">
              {hasLogo ? (
                <img src={withBaseUrl(logoPath)} alt={name} loading="lazy" />
              ) : (
