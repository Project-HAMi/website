...
1. If your image is not a 'dtk-embedded-image', you need to install `pciutiils`, `libelf-dev`, `kmod` after task running, otherwise, dcu-tools like `hy-smi` or `hy-virtual` may not work properly

2. DCU-sharing in init container is not supported, pods with "hygon.com/dcumem" in init container will never be scheduled.

3. Only one vdcu can be acquired per container. If you want to mount multiple dcu devices, then you shouldn't set `hygon.com/dcumem` or `hygon.com/dcucores`
