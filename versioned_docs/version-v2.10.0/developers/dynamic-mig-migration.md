---
title: Migrating to HAMi Dynamic MIG
sidebar_label: Dynamic MIG Migration
---
...
The `nvidia.com/mig-profile-preference` annotation is not supported in v2.10.0. It was added later in [HAMi PR #3014](https://github.com/Project-HAMi/HAMi/pull/3014). In v2.10.0, configure the allowed profiles using `migProfileAllowlist`; do not rely on a Pod preference annotation.
...
Usually not. Users continue to request `nvidia.com/gpu` and `nvidia.com/gpumem` and select `nvidia.com/vgpu-mode: "mig"`. The Pod-level `nvidia.com/mig-profile-preference` annotation is not supported in v2.10.0; see [Preferring a MIG profile](#preferring-a-mig-profile). The scheduler and device plugin manage `hami.io/vgpu-mig-allocations`; it is not a user-facing API.
