...
- 能够通过 `kubectl` 和 Helm 访问集群，并有权安装 HAMi、标记节点。在 GPU 节点上有权使用 `sudo` 检查配置和管理 systemd 服务。
...
在每个 GPU 节点上执行以下步骤。根据 Toolkit 由宿主机还是 GPU Operator 管理，选择对应的配置步骤，再持久保存配置并重启 K3s 服务。

### 检查 containerd 路径

K3s 会为每个节点[生成 containerd 配置](https://docs.k3s.io/advanced#configuring-containerd)。配置工具必须使用 K3s 内置 containerd 的配置文件和 socket，默认路径如下：
...
K3s 启动时通过服务进程的 `PATH` 查找运行时可执行文件。如果安装运行时时 K3s 已在运行，需要重启 K3s，使其检测新安装的运行时。参见 [K3s 运行时支持说明](https://docs.k3s.io/advanced#alternative-container-runtime-support)。

先在 GPU 节点上查找运行时可执行文件：
...
确认导入规则包含 `99-nvidia.toml`，且该文件指定的运行时可执行文件存在。此处的补充配置文件路径适用于本文的 v3 配置；使用其他版本或自定义模板时，先检查实际导入规则。

### 持久保存配置

K3s 会生成 `config.toml`，直接修改该文件无法保证配置在重启后仍然有效。
...
运行时配置应在每个承载 GPU 工作负载的节点上检查。重启 K3s 会影响节点服务。单节点集群的 Server 节点重启期间，Kubernetes API 会短暂不可用。

参照 [K3s 服务重启说明](https://docs.k3s.io/upgrades/manual#upgrade-k3s-using-the-binary)，根据 GPU 节点的角色，在该节点上执行对应命令：
...
K3s [预置受支持运行时的 RuntimeClass 定义](https://docs.k3s.io/advanced#nvidia-container-runtime)。仅有 `RuntimeClass/nvidia`，还不能确认节点已配置 NVIDIA 运行时。重启后，还需检查前文的主配置及导入的文件，确认配置中仍有该运行时，且对应的可执行文件存在。

节点恢复 `Ready` 且运行时配置存在后，继续安装 HAMi。

## 安装 HAMi

按[前置条件中的节点标记步骤](./prerequisites.md)为 GPU 节点添加 `gpu=on`，再按[在线安装](./online-installation.md)部署 HAMi。保留所选 NVIDIA 部署方案的 Helm values。
...
| Toolkit 更新运行时配置时，K3s API 短暂不可用 | 按时间对照 Toolkit 日志与 K3s 服务日志，检查 containerd 是否退出、K3s 是否重启，以及服务随后是否恢复 |
