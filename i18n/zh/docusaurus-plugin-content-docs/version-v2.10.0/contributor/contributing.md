...
2. 处理所有审查意见。如果你不认同某条反馈，在讨论区说明原因。回复必须由你自己撰写，并针对具体提出的问题，逐字照搬或套话式的 AI 回复如果没有回应到评论内容，PR 会被关闭。
...
可以使用 AI 工具辅助编写代码、文档或提交信息，但有两条硬性规则：

**如果你使用了任何形式的 AI 辅助，必须在 PR 描述中披露，并说明使用程度（仅用于文档，还是也用于生成代码）。** 一句话说明即可，例如"这个 PR 主要由 Claude Code 编写"，或"我用 ChatGPT 理解了代码库，但改动是我自己完成的"。这条规则和下面这条是分开的：即便 PR 完全由你自己写、AI 只用来检查语法，也建议加一句说明，方便审阅者判断该投入多深的审查力度。
...
如果 AI 辅助的改动超出小修小补的范围，请先开一个 issue，并将改动拆分成可审查的多个提交；未经这一步就提交的大型 AI 生成 PR 不会被接受。不要把 AI 工具列为提交的共同作者或添加相关 trailer（例如 `assisted-by`、`co-developed-by` 等），披露信息只应出现在 PR 描述中。
...
- 尽可能在真实硬件上测试。CI 可以接受模拟测试，但新后端合并前需要有硬件验证。请在 PR 或 lab 文档中记录测试了什么、设备型号和驱动版本。
...
如果维护者或贡献者在评论或审查中要求回应，作者有一周时间回复。超过一周没有回应，该 issue 或 PR 会被关闭。关闭不是永久性的，如果这项工作仍然需要，随时可以重新打开或重新提交。这样可以让待办事项保持聚焦，避免过时的工作挡住活跃的贡献者。
