...
v2.10.0 的 device plugin 在构造插件时初始化 `MigInstanceManager`，并在插件的 context 结束时关闭管理器。这种按插件生命周期持有会话的方式由 [HAMi PR #2610](https://github.com/Project-HAMi/HAMi/pull/2610) 引入。

每个启动周期单独初始化、等待已接受操作执行完毕以及安全地重复关闭，是之后的 [HAMi PR #2989](https://github.com/Project-HAMi/HAMi/pull/2989) 添加的。上述这些后续版本的生命周期保证不适用于 v2.10.0。
