...
![Metax GPU 拓扑图，显示 PCIe Switch 和 MetaXLink 连接](/img/docs/common/userguide/metax-device/metax-gpu/metax-topology.jpg)
...
2. 使用 `node-scheduler-policy=spread` 时，尽可能将 Metax 资源分配在同一个 Metaxlink 或 Paiswich 下，如下图所示：
   ...
   ![Metax spread 调度策略图，展示资源分配](/img/docs/common/userguide/metax-device/metax-gpu/metax-spread.jpg)

3. 使用 `node-scheduler-policy=binpack` 时，分配 GPU 资源，以尽量减少对 MetaxXLink 拓扑的破坏，如下图所示：

   ![Metax binpack 调度策略图，展示拓扑感知分配](/img/docs/common/userguide/metax-device/metax-gpu/metax-binpack.jpg)
...
- 在 metax 节点上部署沐曦 GPU 插件（请咨询你的设备提供商以获取其软件包和文档）
