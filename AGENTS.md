...
- `versioned_docs/version-vX.Y.Z/` and `i18n/zh/.../version-vX.Y.Z/` are **immutable frozen snapshots** generated by the versioning command. Never hand-copy files into them to create a new version snapshot; see the targeted fix workflow below for changes to an already-released version.
- The first entry in `versions.json` is the default version served at `/docs` (the **latest stable release**, e.g. v2.9.0, not the `current/` directory, which is the unreleased dev version).
