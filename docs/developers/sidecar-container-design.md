...
- **Usage recording:** `CollapseInitContainerUsage` routes sidecars into the app-sum bucket and adds their sum to the init peak. The same split applies to the per-entry slot count ([HAMi#2623](https://github.com/Project-HAMi/HAMi/pull/2623)): sidecar slots add like app containers, non-sidecar inits keep their peak of 1. `getNodesUsage` only consumes the stored output, so it needs no change of its own. Add/update/delete symmetry stays as it is.
